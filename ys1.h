...
int32_t MapScriptHook() {
    ...
    if (bossTargetPtr == 0 || ReadValue<char>(AdolHP + 0x298) == 0)
        bossTargetPtr = adol;
...
void __fastcall Ys1MovementHook(void* arg1) {
    ...
    if ((int)arg1 == feena) {
    ...
    else if (GetKeyState(SAVE) & 0x8000 && savedTimer == 0 && ReadValue<char>(AdolHP + 0x298) == 0) {
        int SaveSlot = baseAddress + 0x135664;
        ...
        bool saved = ((OrigFunc)originalFunc)();
        originalFunc = (void*)(baseAddress + 0xA58E0);      //Game data save
        if (saved && ((OrigFunc)originalFunc)()) {
            savedTimer = 90;
        }
    }
...
void __fastcall Ys1DamageHook(uint32_t arg1) {
	...
	WriteBytes(arg1 + 0x78, (int)arg1 == adol ? (savedTimer > 0 ?
        "\x25\x00\x00\x00\x02\x00\x00\x00\x14\x00\x00\x00\xE1\xFF\xFF\xFF\x00\x00\x00\x00\x20\x20\x20\x20\x53\x41\x56\x45\x44\x00\x00\x00" :
	...
	if ((int)arg1 == adol) {
        ...
        if (ReadValue<int>(baseAddress + 0x1313B0) <= 0 && ReadValue<int>(FeenaData) < baseAddress)
            ...
            if (result && feena >= baseAddress) {
				...
				memcpy((void*)feena, (void*)adol, 0x480);
                WriteValue<char>(feena + 4, (FeenaData-AdolData)/4);
                //if (room != 114) {
...
public:
    void main() {
        int FeenaActive = baseAddress + 0x131C44,
            ...
            Boss2TargetCode = baseAddress + 0x3122D,
            Boss3TargetCode = baseAddress + 0x3CD70,
            Boss3TargetCode2 = baseAddress + 0x3DCC8,
            Boss4TargetCode = baseAddress + 0x3B9A3,
            Boss4TargetCode2 = baseAddress + 0x3A62A,
        ...
        *(int*)bytes = (int)&bossTargetPtr;
        ...
        WriteBytes(Boss2TargetCode, bytes, 4);
        WriteBytes(Boss3TargetCode, bytes, 4);
        WriteBytes(Boss3TargetCode2, bytes, 4);
        WriteBytes(Boss4TargetCode, bytes, 4);
        WriteBytes(Boss4TargetCode2, bytes, 4);
